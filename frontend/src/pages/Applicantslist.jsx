...
const ApplicantsList = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4000/api/jobs/company-jobs", {
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    const matchesStatus = filterStatus === "All" || (filterStatus === "Active" ? job.isActive : !job.isActive);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative flex antialiased selection:bg-indigo-500 selection:text-white">
      ...
      {/* -------------------- SIDEBAR (Synced) -------------------- */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900/60 backdrop-blur-xl border-r border-slate-800/60 transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-slate-800/40">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-extrabold tracking-tight">CareerConnect</h1>
                <p className="text-[10px] font-mono text-indigo-400 font-bold uppercase tracking-widest">Recruit OS</p>
              </div>
            </div>
          </div>
          ...
          <nav className="p-4 flex-1 space-y-1.5">
            {[
              ...
              { name: "Applicants List", icon: LayoutDashboard, path: "/company-dashboard/applicants/6a1001d9d2dd8908ed7b591c" },
              { name: "Active Jobs", icon: LayoutDashboard, path: "/company-dashboard/applicants-list" },
          ...
          <div className="p-4 border-t border-slate-800/40">
            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold text-rose-400 border border-rose-500/10 hover:bg-rose-500/10 transition-all">
        ...
        <header className="h-20 border-b border-slate-800/40 bg-slate-950/60 backdrop-blur-md sticky top-0 z-40 px-6 sm:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-slate-400">
              <Briefcase className="w-4 h-4" /> / <span className="text-white">Active Listings</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
             <div className="hidden sm:block text-right">
                <p className="text-xs font-bold text-slate-200">{displayName}</p>
                <p className="text-[10px] text-slate-500 font-mono font-bold uppercase">Employer Account</p>
             </div>
        ...
        <main className="p-4 sm:p-8 max-w-6xl mx-auto w-full space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-1">
              <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Manage Job Vectors</h2>
              <p className="text-sm text-slate-500 font-medium">Monitoring {jobs.length} deployment channels across the talent pipeline.</p>
            </div>
            <button onClick={() => navigate("/company-dashboard")} className="px-5 py-2.5 bg-linear-to-r from-indigo-600 to-purple-600 text-white rounded-xl text-xs font-bold shadow-lg shadow-indigo-500/20 border border-white/10 transition-all hover:scale-105 active:scale-95 flex items-center gap-2">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-900/40 p-4 rounded-2xl border border-slate-800/60 backdrop-blur-md">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <button onClick={fetchJobs} className="p-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-500 hover:text-white transition-all">
          ...
          <div className="grid grid-cols-1 gap-4">
            {loading ? (
              [1, 2, 3].map(i => <div key={i} className="h-32 bg-slate-900/20 rounded-2xl border border-slate-800/40 animate-pulse" />)
              ...
              <div className="py-24 text-center border-2 border-dashed border-slate-900/60 rounded-3xl space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Modify your search query or status filter nodes.</p>
                </div>
              </div>
            ) : (
              filteredJobs.map((job) => (
                <div key={job._id} className="group p-6 bg-slate-900/20 border border-slate-800/60 hover:border-indigo-500/40 hover:bg-slate-900/40 rounded-2xl transition-all duration-300 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xs hover:shadow-indigo-500/5">
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center flex-wrap gap-3 text-[10px] text-slate-500 font-mono font-bold uppercase tracking-wide">
                        ...
                        <span className="text-slate-800">|</span>
                        <span className="text-purple-400">{job.employmentType}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-8 w-full md:w-auto justify-between md:justify-end">
                    ...
                    <div className="text-left sm:text-right space-y-1.5 min-w-30">
                      ...
                      <div className="flex items-center sm:justify-end gap-3">
                        <span className="text-2xl font-black text-white">{job.totalApplicants || 0}</span>
                        <div className="w-12 h-1 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                          <div className="h-full bg-linear-to-r from-indigo-500 to-purple-500 shadow-[0_0_10px_rgba(99,102,241,0.4)]" 
                               style={{ width: `${Math.min(((job.totalApplicants || 0) / 50) * 100, 100)}%` }} />
                        </div>
                      </div>
                    </div>

                    {/* Status Pill */}
                    <div className={`px-4 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 ${job.isActive ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.05)]' : 'bg-slate-800 text-slate-500 border-slate-700'}`}>
                      <CircleDot className={`w-3 h-3 ${job.isActive ? 'animate-pulse' : ''}`} />
                    ...
                    <div className="flex items-center gap-3">
                      <button 
                        onClick={() => navigate(`/company-dashboard/applicants/${job._id}`)}
                      ...
                      >
                        Candidates <ArrowUpRight className="w-3.5 h-3.5 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform" />
                      </button>
