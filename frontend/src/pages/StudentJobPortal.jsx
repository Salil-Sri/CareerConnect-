...
export default function StudentJobPortal() {
      ...
      if (data.success) {
        ...
        if (studentId) {
          ...
          data.jobs.forEach(job => {
            const hasApplied = job.applicants?.some(app => app.student === studentId || app.student?._id === studentId);
            if (hasApplied) alreadyApplied.add(job._id);
          });
  ...
  const handleApply = async (jobId) => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/jobs/apply/${jobId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-900 pb-6">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-indigo-400 bg-indigo-500/10 px-2.5 py-1 rounded-md border border-indigo-500/20 max-w-fit font-bold mb-2 uppercase tracking-wider">
              <Cpu className="size-3.5 animate-spin" /> AI Screening Active Marketplace
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
              Explore Live Openings
            </h1>
            <p className="text-slate-400 text-xs sm:text-sm mt-1">
              Your synced profile score determines position priority in corporate verification lists.
            </p>
          </div>
          <button onClick={fetchJobs} className="flex items-center gap-2 text-xs font-bold bg-slate-900 border border-slate-800 hover:border-slate-700 hover:text-white text-slate-300 px-4 py-2.5 rounded-xl transition-all cursor-pointer">
            <RefreshCw className={`size-3.5 ${loading ? "animate-spin text-indigo-400" : ""}`} /> Sync Database Feed
          </button>
        </div>
        ...
        <div className="p-4 bg-slate-900/40 backdrop-blur-md border border-slate-800/60 rounded-2xl grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              ...
              {["Internship", "Fresher", "Junior", "Mid-Level", "Senior"].map(exp => (
        ...
        {loading ? (
          ...
          <div className="p-16 border border-dashed border-slate-800/80 rounded-2xl text-center max-w-md mx-auto space-y-2">
            ...
            <h3 className="text-sm font-bold text-slate-300">No Target Vectors Found</h3>
            <p className="text-xs text-slate-500 font-medium">Try modifying your text terms or structural navigation criteria parameters.</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                          <Building2 className="size-3.5 text-slate-500" /> {job.companyName}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-semibold text-slate-400 pt-1">
                      <div className="flex items-center gap-1.5 truncate"><MapPin className="size-3.5 text-slate-600" /> {job.location}</div>
                      <div className="flex items-center gap-1.5 truncate"><DollarSign className="size-3.5 text-slate-600" /> {job.salary}</div>
                      <div className="flex items-center gap-1.5 truncate"><GraduationCap className="size-3.5 text-slate-600" /> {job.experienceLevel}</div>
                      <div className="flex items-center gap-1.5 text-slate-500 font-mono text-[10px]"><Clock className="size-3.5 text-slate-600" /> {new Date(job.createdAt).toLocaleDateString()}</div>
                    </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-900/60">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                      ...
                      >
                        {applyingId === job._id ? (
                          <>
                            <RefreshCw className="size-3.5 animate-spin" /> Transmitting Vector...
                          </>
                        ) : isApplied ? (
                          <>
                            <CheckCircle2 className="size-3.5 text-emerald-400" /> Application Lodged
                          </>
                        ) : (
                          <>
                            Apply Instantly <ArrowUpRight className="size-3.5" />
                          </>
                        )}
                      </button>
